...
export function Sidebar({ onAddNode }: SidebarProps) {
  ...
  const categoryConfig = {
    ...
    security: {
      title: "Security",
      description:
        "Configure security features like rate limiting, CORS, and CSRF protection.",
      icon: <LockIcon className="w-6 h-6" />,
      color: "bg-red-600",
    },
    configuration: {
      title: "Configuration",
      description:
        "Advanced settings for sessions, cookies, and other authentication options.",
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        >
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            {selectedCategory && (
              <div className="mt-4">
                {groupedNodes[selectedCategory] &&
                groupedNodes[selectedCategory].length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    <div className="text-muted-foreground mb-2">
                      {selectedCategory === "database" &&
                        "No database components available"}
                      {selectedCategory === "authentication" &&
                        "No authentication components available"}
                      {selectedCategory === "plugins" &&
                        "Plugin components coming soon"}
                      {selectedCategory === "security" &&
                        "Security components coming soon"}
                      {selectedCategory === "configuration" &&
                        "Configuration components coming soon"}
                    </div>
                    <p className="text-sm text-muted-foreground">
                      These components will be available for code generation but
                      are not yet draggable.
                    </p>
                  </div>
                )}
              </div>
